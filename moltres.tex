...
mesh convergence as further mesh refinement by a factor of two produced a small
0.1 pcm increase in the reactivity for Step 0.2. We adopted the group constant
data provided by Tiberga et al. \cite{tiberga_results_2020}. Next, we
...
which we discretized using zeroth-order monomial shape functions for the 
\gls{DGFEM} solver mentioned in Section 3.1. Zeroth-order \gls{DGFEM} shares
the same order of accuracy as first-order \gls{FVM} because they share the same
...
and the thermal-hydraulics due to the unique problem setups involving
neutronics criticality calculations for the neutron multiplication factor
