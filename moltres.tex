...
\cite{dehart_reactor_2011} or Serpent 2 \cite{leppanen_serpent_2014}. Users can
...
\begin{align}
    ...
    \sum^G_{g'=1} \left( 1-\beta \right) \nu \Sigma^f_{g'} \phi_{g'} \nonumber
    \\
    &+ \chi^d_g \sum^I_i \lambda_i C_i, \label{eq:neut} \\
    %
    \intertext{where}
    ...
    scattering from groups $g'$ to $g$ [cm$^{-1}$],} \nonumber \\
...
\begin{align}
    ...
    \nonumber \\
    \lambda_i =& \text{ average decay constant of delayed neutron precursors
    in} \nonumber \\
    &\text{ precursor group $i$ [s$^{-1}$],} \nonumber \\
...
\begin{align}
    ...
    %
    \intertext{where}
    Q_f =& \sum^G_{g=1} \epsilon_g \Sigma_g^f
    \phi_g = \text{ fission heat source [W$\cdot$m$^{-3}\cdot$s$^{-1}$],}
    \label{eq:source} \\
...
\begin{align}
    ...
    -\rho (\vec{u}
    \cdot \nabla) \vec{u} - \nabla p + \mu \nabla \cdot \nabla^2 \vec{u}
    + \alpha \vec{g} \rho \left(T - T_{\text{ref}} \right)
    \label{eq:momemtum} \\
    %
    \nabla \cdot \vec{u} =& 0
    \label{eq:divergence}
...
methods in \gls{MOOSE}'s Navier-Stokes module.
